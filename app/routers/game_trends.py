...
from app.models.game_trend import create_game_trend_model
from app.enums.trend_enums import CategoryEnum, MonthEnum, DayOfWeekEnum

router = APIRouter()

# Cache for game filter options to avoid frequent DB queries
_game_options_cache = {}

def validate_table_name(table_name: str) -> bool:
    """
    Validate that the table name follows the expected format.
    Expected format: {home_team}{away_team}{date} (e.g., phidal20250904, lacla20250904)
    Team abbreviations can be 2 or 3 letters each, followed by 8 digits for the date.
    """
    pattern = r'^[a-z]{2,3}[a-z]{2,3}\d{8}$'
...
class TrendFilter(BaseModel):
    ...
    # VALIDATE LIMIT is between 1 and 5000
...
@router.post("/{table_name}", summary="Retrieve game trends with filters", tags=["Game Trends"])
def get_game_trends(
    table_name: str,
    filters: TrendFilter,
    ...
    """
    Retrieve trends from a specific game table based on the provided filters.
    
    Table names should be in format: {home_team}{away_team}{date} (e.g., phidal20250904)
    ...
    # Create dynamic GameTrend model for this table
    GameTrend = create_game_trend_model(table_name)

    query = db.query(GameTrend)
    filters_list = []

    # Filter by TREND ID
    if filters.trend_id:
        if isinstance(filters.trend_id, str):
            filters_list.append(GameTrend.id_string == filters.trend_id)
        else:
            print(filters.trend_id)
            filters_list.append(GameTrend.id_string.in_(filters.trend_id))

    # Filter by CATEGORY
    if filters.category:
        if isinstance(filters.category, str):
            filters_list.append(GameTrend.category == filters.category)
        else:
            filters_list.append(GameTrend.category.in_(filters.category))

    # Filter by MONTH
    month_case = case(
        (GameTrend.month == "January", 1),
        (GameTrend.month == "February", 2),
        (GameTrend.month == "March", 3),
        (GameTrend.month == "April", 4),
        (GameTrend.month == "May", 5),
        (GameTrend.month == "June", 6),
        (GameTrend.month == "July", 7),
        (GameTrend.month == "August", 8),
        (GameTrend.month == "September", 9),
        (GameTrend.month == "October", 10),
        (GameTrend.month == "November", 11),
        (GameTrend.month == "December", 12),
        else_=0
    )
    ...
    if filters.month:
        if isinstance(filters.month, str):
            if filters.month == "None":
                month_conditions.append(GameTrend.month == None)
            else:
                month_conditions.append(GameTrend.month == filters.month)
        else:  # list
            values = [v for v in filters.month if v != "None"]
            if values:
                month_conditions.append(GameTrend.month.in_(values))
    ...
    if filters.start_month and filters.end_month:
        ...
        range_condition = and_(GameTrend.month != None, month_case.between(start, end))
    elif filters.start_month:
        start = MONTH_MAPPING[filters.start_month]
        range_condition = and_(GameTrend.month != None, month_case >= start)
    elif filters.end_month:
        end = MONTH_MAPPING[filters.end_month]
        range_condition = and_(GameTrend.month != None, month_case <= end)
    ...
    if filters.day_of_week:
        if isinstance(filters.day_of_week, str):
            if filters.day_of_week == 'None':
                filters_list.append(GameTrend.day_of_week.is_(None))
            else:
                filters_list.append(GameTrend.day_of_week == filters.day_of_week)
        else:
            days = []
            include_null = False
            ...
            conditions = []
            if days:
                conditions.append(GameTrend.day_of_week.in_(days))
            if include_null:
                conditions.append(GameTrend.day_of_week.is_(None))
            filters_list.append(or_(*conditions))

    # Filter by DIVISIONAL
    if filters.divisional == "None":
        filters_list.append(GameTrend.divisional.is_(None))
    elif filters.divisional in (True, False):
        filters_list.append(GameTrend.divisional == filters.divisional)
    ...
    if filters.spread:
        ...
        if isinstance(filters.spread, str):
            if filters.spread == "None":
                spread_clauses.append(GameTrend.spread.is_(None))
            ...
            if filters.spread.exact:
                ...
                if non_null_spreads:
                    spread_clauses.append(GameTrend.spread.in_(non_null_spreads))
            ...
            if filters.spread.or_less is not None:
                ...
                spread_clauses.append(GameTrend.spread.in_(or_less_values))
            ...
            if filters.spread.or_more is not None:
                ...
                spread_clauses.append(GameTrend.spread.in_(or_more_values))
    ...
    if filters.total:
        ...
        if isinstance(filters.total, str):
            if filters.total == "None":
                total_clauses.append(GameTrend.total.is_(None))
            ...
            if filters.total.exact:
                ...
                if non_null_totals:
                    total_clauses.append(GameTrend.total.in_(non_null_totals))
                if "None" in exact_list:
                    total_clauses.append(GameTrend.total.is_(None))
            ...
            if filters.total.or_less is not None:
                ...
                or_less_values = [f"{val} or less" for val in or_less_list]
                total_clauses.append(GameTrend.total.in_(or_less_values))
            ...
            if filters.total.or_more is not None:
                ...
                or_more_values = [f"{val} or more" for val in or_more_list]
                total_clauses.append(GameTrend.total.in_(or_more_values))
    ...
    if filters.seasons:
        ...
        if filters.seasons.exact:
            season_clauses.append(GameTrend.seasons.in_(filters.seasons.exact))

        if filters.seasons.since_or_later:
            index = VALID_SEASONS.index(filters.seasons.since_or_later)
            season_clauses.append(GameTrend.seasons.in_(VALID_SEASONS[index:]))

        if filters.seasons.since_or_earlier:
            index = VALID_SEASONS.index(filters.seasons.since_or_earlier)
            season_clauses.append(GameTrend.seasons.in_(VALID_SEASONS[: index + 1]))
    ...
    if filters.wins:
        if isinstance(filters.wins, int):
            filters_list.append(GameTrend.wins == filters.wins)
        else:
            filters_list.append(GameTrend.wins.in_(filters.wins))

    if filters.min_wins is not None and filters.max_wins is not None:
        filters_list.append(GameTrend.wins.between(filters.min_wins, filters.max_wins))
    elif filters.min_wins is not None:
        filters_list.append(GameTrend.wins >= filters.min_wins)
    elif filters.max_wins is not None:
        filters_list.append(GameTrend.wins <= filters.max_wins)

    # Filter by LOSSES
    if filters.losses:
        if isinstance(filters.losses, int):
            filters_list.append(GameTrend.losses == filters.losses)
        else:
            filters_list.append(GameTrend.losses.in_(filters.losses))

    if filters.min_losses is not None and filters.max_losses is not None:
        filters_list.append(GameTrend.losses.between(filters.min_losses, filters.max_losses))
    elif filters.min_losses is not None:
        filters_list.append(GameTrend.losses >= filters.min_losses)
    elif filters.max_losses is not None:
        filters_list.append(GameTrend.losses <= filters.max_losses)

    # Filter by PUSHES
    if filters.pushes:
        if isinstance(filters.pushes, int):
            filters_list.append(GameTrend.pushes == filters.pushes)
        else:
            filters_list.append(GameTrend.pushes.in_(filters.pushes))

    if filters.min_pushes is not None and filters.max_pushes is not None:
        filters_list.append(GameTrend.pushes.between(filters.min_pushes, filters.max_pushes))
    elif filters.min_pushes is not None:
        filters_list.append(GameTrend.pushes >= filters.min_pushes)
    elif filters.max_pushes is not None:
        filters_list.append(GameTrend.pushes <= filters.max_pushes)

    # Filter by TOTAL GAMES
    if filters.total_games:
        if isinstance(filters.total_games, int):
            filters_list.append(GameTrend.total_games == filters.total_games)
        else:
            filters_list.append(GameTrend.total_games.in_(filters.total_games))

    if filters.min_total_games is not None and filters.max_total_games is not None:
        filters_list.append(GameTrend.total_games.between(filters.min_total_games, filters.max_total_games))
    elif filters.min_total_games is not None:
        filters_list.append(GameTrend.total_games >= filters.min_total_games)
    elif filters.max_total_games is not None:
        filters_list.append(GameTrend.total_games <= filters.max_total_games)

    # Filter by WIN PERCENTAGE
    if filters.win_percentage:
        if isinstance(filters.win_percentage, float):
            filters_list.append(GameTrend.win_percentage == filters.win_percentage)
        else:
            filters_list.append(GameTrend.win_percentage.in_(filters.win_percentage))

    if filters.min_win_percentage is not None and filters.max_win_percentage is not None:
        filters_list.append(GameTrend.win_percentage.between(filters.min_win_percentage, filters.max_win_percentage))
    elif filters.min_win_percentage is not None:
        filters_list.append(GameTrend.win_percentage >= filters.min_win_percentage)
    elif filters.max_win_percentage is not None:
        filters_list.append(GameTrend.win_percentage <= filters.max_win_percentage)
    ...
    # Sorting
    valid_sort_fields = {c_attr.key for c_attr in inspect(GameTrend).mapper.column_attrs}
    if filters.sort_by:
        sort_columns = []
        for sort in filters.sort_by:
            ...
            if sort.field == "month":
                month_case = case(
                    ...
                    (GameTrend.month == "November", 11),
                    (GameTrend.month == "December", 12),
                    (GameTrend.month == None, 13),
                    else_=14
                )
                sort_columns.append(month_case.desc() if sort.order == "desc" else month_case.asc())
            elif sort.field == 'day_of_week':
                day_case = case(
                    (GameTrend.day_of_week == "Sunday", 1),
                    (GameTrend.day_of_week == "Monday", 2),
                    (GameTrend.day_of_week == "Tuesday", 3),
                    (GameTrend.day_of_week == "Wednesday", 4),
                    (GameTrend.day_of_week == "Thursday", 5),
                    (GameTrend.day_of_week == "Friday", 6),
                    (GameTrend.day_of_week == "Saturday", 7),
                    (GameTrend.day_of_week == None, 8),
                    else_=9
                )
                sort_columns.append(day_case.desc() if sort.order == "desc" else day_case.asc())
            else:
                col = getattr(GameTrend, sort.field)
...
@router.get("/", summary="List available game trend tables", tags=["Game Trends"])
def list_available_tables(db: Session = Depends(get_connection)):
    """
    List all available game trend tables.
    """
    try:
        inspector = inspect(db.bind)
        all_tables = inspector.get_table_names()
        
        # Filter for game trend tables (pattern: teamteamyyyymmdd)
        game_tables = [table for table in all_tables if validate_table_name(table)]
        
        return {
            "available_tables": game_tables,
            "total_count": len(game_tables)
        }
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Error listing tables: {str(e)}")

@router.get("/{table_name}/filter-options", summary="Get available filter options for a specific game", tags=["Game Trends"])
def get_game_filter_options(table_name: str, db: Session = Depends(get_connection)):
    """
    Get available filter options for a specific game table based on current data.
    Returns the distinct values available in the game trends table.
    ...
    # Check if we have cached data that's still valid (5 minute TTL)
    cache_key = table_name
    current_time = time.time()
    ttl = 300  # 5 minutes
    
    if (cache_key in _game_options_cache and 
        _game_options_cache[cache_key]["data"] is not None and 
        current_time - _game_options_cache[cache_key]["timestamp"] < ttl):
        return _game_options_cache[cache_key]["data"]
    ...
    try:
        ...
        months = [row[0] for row in db.query(func.distinct(GameTrend.month)).all() if row[0] is not None]
        day_of_weeks = [row[0] for row in db.query(func.distinct(GameTrend.day_of_week)).all() if row[0] is not None]
        divisionals = [row[0] for row in db.query(func.distinct(GameTrend.divisional)).all() if row[0] is not None]
        spreads = [row[0] for row in db.query(func.distinct(GameTrend.spread)).all() if row[0] is not None]
        totals = [row[0] for row in db.query(func.distinct(GameTrend.total)).all() if row[0] is not None]
